...
Route::middleware(['auth', 'role:teknisi'])->prefix('teknisi')->group(function () {
    // Dashboard teknisi → hanya tampilkan laporan dengan status 'dikirim'
    ...
    Route::get('/laporan/{id}', [TeknisiLaporanController::class, 'show'])
        ->name('teknisi.laporan.show');

    // Update status & notes laporan
    Route::put('/laporan/{id}', [TeknisiLaporanController::class, 'update'])
        ->name('teknisi.laporan.update');
});
