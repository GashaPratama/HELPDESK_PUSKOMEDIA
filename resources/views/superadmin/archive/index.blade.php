              ...
              @foreach($archivedTickets as $ticket)
                <tr class="hover:bg-gray-50 transition-colors">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div class="flex items-center space-x-2">
                      <form method="POST" action="{{ route('superadmin.archive.restore', $ticket->id) }}" class="inline">
                        @csrf
                        <button type="submit" 
                      ...
                      <form method="POST" action="{{ route('superadmin.archive.force-delete', $ticket->id) }}" class="inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit" 
...
function bulkRestore() {
  ...
  if (confirm(`Yakin ingin memulihkan ${selectedTickets.length} tiket dari arsip?`)) {
    ...
    form.method = 'POST';
    form.action = '{{ route("superadmin.archive.bulk-restore") }}';
...
function bulkForceDelete() {
  ...
  if (confirm(`Yakin ingin menghapus ${selectedTickets.length} tiket secara permanen? Tindakan ini tidak dapat dibatalkan!`)) {
    const form = document.createElement('form');
    form.method = 'POST';
    form.action = '{{ route("superadmin.archive.bulk-force-delete") }}';
